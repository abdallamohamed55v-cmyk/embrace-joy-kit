...
function getAllowedDurations(model: DurationModel): number[] {
  if (model.slug?.startsWith("veo-3")) return [4, 6, 8];
...
function aspectToSize(aspect: string, res: string): { width: number; height: number; mode: "RESOLUTION_720" | "RESOLUTION_1080" } {
  ...
  if (!a || !b) return { width: 1280, height: base, mode };
  if (a >= b) return { width: Math.round((base * a) / b), height: base, mode };
  return { width: base, height: Math.round((base * b) / a), mode };
}
