...
const INTENTS: { id: Intent; label: string; icon: typeof MessageSquare; placeholder: string }[] = [
  { id: "chat", label: "محادثة", icon: MessageSquare, placeholder: "اسأل أي شيء... مثلاً: لخّص لي أحدث أخبار الذكاء الاصطناعي" },
  { id: "slides", label: "عرض تقديمي", icon: Presentation, placeholder: "صمم لي عرضاً عن مستقبل التعليم بـ 10 شرائح" },
  { id: "docs", label: "مستند", icon: FileText, placeholder: "اكتب لي خطة عمل مفصّلة لمشروع تطبيق توصيل" },
];
    ...
    >
      <div className="rounded-2xl border border-border/60 bg-card/80 backdrop-blur-md shadow-2xl shadow-black/20 overflow-hidden">
        ...
        <div className="flex items-center gap-1 px-2 pt-2">
          ...
          <span className="mr-auto inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/15 text-emerald-600 dark:text-emerald-400 border border-emerald-500/30">
        ...
        <textarea
          ...
          rows={2}
          className="w-full px-4 py-3 bg-transparent border-none outline-none focus:ring-0 resize-none text-sm text-foreground placeholder:text-muted-foreground/60 text-right"
        />
      ...
      <p className="mt-2 text-center text-[11px] text-muted-foreground">
        من غير بطاقة ائتمان • التسجيل في ثانية واحدة بعد ما تشوف النتيجة
      </p>
    </motion.div>
  );
}
