...
export const triggerAha = (key: AhaKey, opts?: { onShare?: () => void; onUpgrade?: () => void }) => {
  ...
  if (!s[key]) {
    ...
    const label: Record<AhaKey, string> = {
      chat: "أول محادثة ناجحة! ✨",
      image: "أول صورة جاهزة! 🎨",
      doc: "أول مستند تم! 📄",
      slide: "أول عرض جاهز! 🎞️",
      video: "أول فيديو ناجح! 🎬",
      code: "أول مشروع كود! 💻",
    };
    toast.success(label[key], {
      description: opts?.onShare ? "شاركه مع أصحابك" : undefined,
      action: opts?.onShare ? { label: "مشاركة", onClick: opts.onShare } : undefined,
    });
  }
  ...
  if (n === 3) {
    setTimeout(() => {
      toast("💎 خصم 50% على Megsy Pro", {
        description: "صور وسلايدز ومستندات بلا حدود — لفترة محدودة",
        duration: 8000,
        action: {
          label: "فعّل الآن",
