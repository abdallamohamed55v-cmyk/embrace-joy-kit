...
const ReferralsPage = () => {
  ...
  const [activeTab, setActiveTab] = useState<"clicks" | "referrals" | "earnings" | "withdrawals">("clicks");
  ...
  const loadData = useCallback(async () => {
    ...
    const { data: clk } = await supabase
      ...
      .select("id, code, utm_source, utm_medium, utm_campaign, country, referer, converted_user_id, created_at")
      .eq("referrer_user_id", user.id)
      .order("created_at", { ascending: false })
      .limit(200);
    if (clk) setClicks(clk as Click[]);
  }, []);
  ...
  const content = (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-8 max-w-lg mx-auto pb-16">
      ...
      {/* Stats — clicks + signups + conversion + earnings */}
      <div className="grid grid-cols-2 gap-4">
        {(() => {
          ...
          return [
            ...
            { label: "Signups", value: referrals.length.toString() },
            { label: "Conv. Rate", value: `${convRate}%` },
            { label: "Total Earned", value: `$${totalEarned.toFixed(2)}` },
          ];
        })().map(stat => (
          <div key={stat.label} className="text-center py-3 rounded-xl bg-muted/20">
      ...
      <div className="text-center">
        <p className="text-[11px] text-muted-foreground">
          Available to withdraw: <span className="text-foreground font-semibold">${availableBalance.toFixed(2)}</span>
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-1">
          {[
            ...
            { step: "2", text: "We track every click — clicks, country, UTM, conversions" },
      ...
      <div className="flex bg-secondary/50 rounded-full p-1">
        {(["clicks", "referrals", "earnings", "withdrawals"] as const).map(t => (
          <button
            key={t}
      ...
      <AnimatePresence mode="wait">
        <motion.div key={activeTab} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.15 }} className="space-y-1">
          {activeTab === "clicks" && (
            clicks.length === 0 ? (
              <p className="text-center text-sm text-muted-foreground py-10">No clicks yet. Share your link to start tracking.</p>
            ) : (
              clicks.map(c => {
                const source = c.utm_source || (c.referer ? new URL(c.referer).hostname.replace(/^www\./, "") : "direct");
                return (
                  <div key={c.id} className="flex items-center justify-between py-3 border-b border-border/50 last:border-0">
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium text-foreground truncate">
                        {source}{c.utm_campaign ? ` · ${c.utm_campaign}` : ""}
                      </p>
                      <p className="text-[11px] text-muted-foreground">
                        {c.country ? `${c.country} · ` : ""}{formatDate(c.created_at)}
                      </p>
                    </div>
                    <span className={`text-[11px] font-medium shrink-0 ml-2 ${c.converted_user_id ? "text-green-500" : "text-muted-foreground"}`}>
                      {c.converted_user_id ? "converted" : "click"}
                    </span>
                  </div>
                );
              })
