...
const ChatPage = () => {
  ...
  const fetchSlidesNarration = async (params: {
    ...
    try {
      const { data, error } = await supabase.functions.invoke("slides-message", { body: params });
    ...
    if (msgs) {
      ...
      setMessages(msgs.map((m: any) => {
        ...
        return {
          ...
          mode: meta.kind === "imageSlides" || (conv as any)?.mode === "slides-images"
            ? "slides-images"
  ...
  const handleSendWithText = async (overrideText?: string) => {
    ...
    // ── Slides + Images mode (2slides.com — image-designed PDF deck) ─
    if (chatMode === "slides-images") {
      ...
      if (!topic || topic.length < 6) {
        toast.error("Please describe your presentation topic clearly, e.g. \"A presentation about ancient Egyptian history in 10 slides\"");
        setIsLoading(false); setIsThinking(false); setSearchStatus("");
        isSubmittingRef.current = false;
      ...
      try {
        ...
        // Freeform "plan" narration BEFORE we kick off the job.
        const planText = await fetchSlidesNarration({
          mode: "plan", topic, kind: "slides-images", title: topic.slice(0, 80),
        });
        if (planText) {
        ...
        if (cid) {
          placeholderId = await saveMessage(cid, "assistant", "", undefined, { kind: "slidesPending", topic: userInput });
          if (placeholderId) ownInsertedIdsRef.current.add(placeholderId);
          if (placeholderId) {
            setMessages((prev) => prev.map((m) =>
              m.clientId === `assistant-${localTurnId}` ? { ...m, id: placeholderId, slidesPendingTopic: userInput, mode: "slides-images" } : m
            ));
          }
          await supabase.from("conversations").update({ updated_at: new Date().toISOString(), mode: "slides-images" } as any).eq("id", cid);
        }
        if (isSlidesRequestCancelled()) return;
        setSearchStatus("Generating slides with images…");
        setIsThinking(true);

        const { data: startData, error: startErr } = await supabase.functions.invoke("slides-api", {
          body: { action: "images_start", topic, language: "Auto" },
        });
        if (isSlidesRequestCancelled()) return;
        if (startErr || !startData?.jobId) {
          throw new Error((startData as any)?.error || startErr?.message || "Failed to start slide generation");
        }
        ...
        while (Date.now() < deadline) {
          ...
          if (isSlidesRequestCancelled()) return;
          const { data: s, error: sErr } = await supabase.functions.invoke("slides-api", {
            body: { action: "images_status", jobId },
          });
          if (sErr) continue;
          if (s?.status === "success" && s?.url) { data = { url: s.url, slideCount: s.slideCount, title: topic.slice(0, 80) }; break; }
          if (s?.status === "failed") throw new Error(s?.message || "Slide generation failed");
        }
        if (!data?.url) {
          throw new Error("Timed out while generating slides");
        }
        const imageSlides = { title: data.title || topic.slice(0, 80), url: data.url as string, slideCount: data.slideCount };
        setMessages((prev) => prev.map((m) =>
          m.clientId === `assistant-${localTurnId}` || (!!placeholderId && m.id === placeholderId)
            ? { ...m, imageSlides, content: "", mode: "slides-images", slidesJobId: undefined }
            : m
        ));
        if (placeholderId) {
          try {
            await supabase.from("messages").update({
              content: "",
              metadata: { kind: "imageSlides", imageSlides } as any,
            }).eq("id", placeholderId);
          } catch { /* best-effort */ }
        }

        // Freeform "summary" narration AFTER the deck is delivered.
        const summaryText = await fetchSlidesNarration({
          mode: "summary", topic, kind: "slides-images",
          title: imageSlides.title, slideCount: imageSlides.slideCount,
        });
        if (summaryText) {
          await insertAssistantNarration(cid, summaryText);
        }
      } catch (e) {
        ...
        setMessages((prev) => prev.map((m) =>
          m.clientId === `assistant-${localTurnId}`
            ? { ...m, content: `Could not generate presentation: ${msg}`, mode: "slides-images" }
            : m
        ));
      } finally {
        setIsLoading(false); setIsThinking(false); setSearchStatus("");
    ...
    if (chatMode === "slides" || shouldAutoStartSlides) {
      const slidesRequestToken = ++slidesGenerationTokenRef.current;
      const isSlidesRequestCancelled = () => slidesGenerationTokenRef.current !== slidesRequestToken;
              ...
              {chatMode === "learning" ? (
                ...
                <div className="flex flex-col">
                  ...
                  {([
                      ...
                      { id: "slides-images", label: "Slides with images", desc: "Presentation designed with images (PDF)", Icon: Image },
                  ...
                  headerSlot={
                    (() => {
                      const CHIPS = [
                        ...
                        {
                          id: "slides-images" as const,
                          label: "Slides with images",
                          activeCls: "bg-background/60 border-border/50 text-pink-600 dark:text-pink-300",
                          inactiveCls: "bg-background/60 border-border/50 text-foreground/65 hover:text-foreground",
                          bubbleCls: "bg-pink-500/15 text-pink-600 dark:text-pink-300",
                          icon: (
                            <Image className="w-3 h-3" strokeWidth={2.4} />
                          ),
                        },
                        {
        ...
        {megsyOsIntroOpen && (
          <>
            ...
            <motion.div
              ...
              className="md:hidden fixed inset-x-0 bottom-0 z-[81] bg-black rounded-t-[32px] border-t border-white/10 pt-3 pb-[calc(env(safe-area-inset-bottom)+1.25rem)] max-w-md mx-auto max-h-[92dvh] overflow-y-auto"
            >
              <div className="pb-4" />

              <div className="px-6 mb-5">
                <h2 style={{ fontFamily: "'Bebas Neue', 'Impact', sans-serif" }} className="text-[52px] leading-[0.88] tracking-tight text-white uppercase">
                  What megsy<br/>
              ...
              <div className="flex gap-4 overflow-x-auto pl-6 pr-6 pb-6 snap-x snap-mandatory [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
                {[
                  ...
                  <div key={role} className={`${bg} shrink-0 w-[280px] snap-center rounded-[28px] p-4 flex flex-col`}>
                    <div className="rounded-[20px] bg-black/5 aspect-square p-8 flex items-center justify-center">{glyph}</div>
                    <div className="px-1 pt-4 pb-1 flex flex-col flex-1">
                      <span className="inline-flex self-start px-2.5 py-1 rounded-full bg-black text-white text-[10px] font-bold tracking-[0.12em] uppercase mb-3">{role}</span>
                      <h3 style={{ fontFamily: "'Bebas Neue', 'Impact', sans-serif" }} className="text-[34px] leading-[0.9] text-black tracking-tight uppercase whitespace-pre-line">{t}</h3>
                      <p className="text-black/75 text-[12.5px] leading-snug mt-2">{d}</p>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="px-6 grid grid-cols-2 gap-3">
                <button onClick={() => setMegsyOsIntroOpen(false)} className="py-3.5 rounded-full bg-black text-white border border-white/15 font-semibold text-[14px] hover:bg-zinc-900 active:scale-[0.98] transition-all">Maybe later</button>
                <button
                  onClick={() => {
